...
class UserAuthenticationController {
  constructor(dependencies = {}) {
    this.auth-service = dependencies.userAuthenticationService;
  ...
  async login(req, res) {
    try {
      ...
      const result = await this.auth-service.authenticateUser(loginIdentifier, password, context);

      // Debug: Log what we got from auth service
      logger.info('[Userauth-controller] Auth result keys:', Object.keys(result));
      logger.info('[Userauth-controller] Has tokens:', !!result.tokens);
      if (result.tokens) {
        logger.info('[Userauth-controller] Has accessToken:', !!result.tokens.accessToken);
      }
      ...
      logger.error('[Userauth-controller] Login error:', error);
  ...
  async refreshToken(req, res) {
    try {
      ...
      const result = await this.auth-service.refreshToken(refreshToken, context);
      ...
      logger.error('[Userauth-controller] Token refresh error:', error);
  ...
  async logout(req, res) {
    try {
      ...
      await this.auth-service.logout(userId, sessionId, context);
      ...
      logger.error('[Userauth-controller] Logout error:', error);
  ...
  async changePassword(req, res) {
    try {
      ...
      await this.auth-service.changePassword(userId, currentPassword, newPassword, context);
      ...
      logger.error('[Userauth-controller] Password change error:', error);
  ...
  async getProfile(req, res) {
    try {
      ...
      logger.error('[Userauth-controller] Get profile error:', error);
  ...
  async updateProfile(req, res) {
    try {
      ...
      logger.error('[Userauth-controller] Update profile error:', error);
  ...
  async forgotPassword(req, res) {
    try {
      ...
      logger.error('[Userauth-controller] Forgot password error:', error);
  ...
  async resetPassword(req, res) {
    try {
      ...
      const passwordValidation = this.auth-service._validatePassword(newPassword);
      ...
      logger.error('[Userauth-controller] Reset password error:', error);

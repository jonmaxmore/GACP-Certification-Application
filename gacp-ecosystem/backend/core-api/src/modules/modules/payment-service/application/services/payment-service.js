...
class payment-service {
  ...
  async calculateApplicationFees(applicationType, options = {}) {
    try {
      ...
      console.log(`[payment-service] Calculating fees for ${applicationType}`, {
        isExpedited,
        requiresInspection,
        promoCode,
      });
      ...
      logger.info('[payment-service] Fee calculation completed:', feeBreakdown);
      return {
        success: true,
        feeBreakdown,
      ...
      logger.error('[payment-service] Fee calculation error:', error);
  ...
  async initiatePayment(paymentRequest, userId) {
    try {
      ...
      console.log(`[payment-service] Initiating payment for application ${applicationId}`, {
        paymentType,
        amount,
        userId,
      });
      ...
      logger.info(`[payment-service] Payment initiated successfully: ${payment.paymentId}`);
  ...
  async processPaymentWebhook(webhookData) {
    try {
      ...
      console.log(`[payment-service] Processing webhook for payment ${paymentId}`, {
        status,
        transactionId,
        amount,
      });
      ...
      logger.error('[payment-service] Webhook processing error:', error);
  ...
  async processRefund(paymentId, refundRequest, adminUserId) {
    try {
      ...
      logger.error('[payment-service] Refund processing error:', error);
      throw error;
    }
  }
  ...
  async _generatePromptPayQR(payment) {
    try {
      ...
      logger.error('[payment-service] QR generation error:', error);

...
class AuthController {
    ...
    async register(req, res) {
        try {
            console.log('[auth-controller] Register Request Recieved');
            ...
            if (req.file && req.file.path) {
                fs.unlink(req.file.path, (err) => {
                    if (err) console.error('[auth-controller] Cleanup Failed:', err.message);
                });
            }
            ...
            if (error.code === 11000 || error.message.includes('E11000')) {
                ...
                console.log('[auth-controller] Duplicate Key Index:', indexName, 'KeyValue:', error.keyValue);
    ...
    async checkIdentifier(req, res) {
        try {
            ...
            console.error('[auth-controller] Check Identifier Error:', error.message);
            res.status(500).json({
                success: false,
                error: 'Server Error',
                available: false

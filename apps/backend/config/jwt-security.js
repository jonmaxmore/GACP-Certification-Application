...
const getJWTConfiguration = () => {
    return {
        public: {
            secret: process.env.FARMER_JWT_SECRET || process.env.JWT_SECRET || 'default-dev-secret-do-not-use-in-prod',
...
const verifyToken = (token, type, config) => {
    ...
    // console.log(`DEBUG JWT VERIFY: Checking ${type} token with secret: ${secret.substring(0, 5)}...`); 
    try {

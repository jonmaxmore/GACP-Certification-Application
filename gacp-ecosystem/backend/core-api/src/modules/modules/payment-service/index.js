...
class payment-serviceModule {
  constructor(dependencies = {}) {
    ...
    logger.info('[payment-serviceModule] Initialized successfully');
  }
  ...
  _initializeModule() {
    try {
      ...
      this.service = new payment-service(this.dependencies);
      ...
      logger.info('[payment-serviceModule] All components initialized');
    } catch (error) {
      logger.error('[payment-serviceModule] Initialization failed:', error);
      throw error;
    }
  }
  ...
  _validateDependencies() {
    ...
    if (!process.env.PROMPTPAY_MERCHANT_ID) {
      logger.warn('[payment-serviceModule] PROMPTPAY_MERCHANT_ID not configured - using default');
    }

    if (!process.env.PROMPTPAY_WEBHOOK_SECRET) {
      console.warn(
        '[payment-serviceModule] PROMPTPAY_WEBHOOK_SECRET not configured - webhook security disabled',
      );
    }
    ...
    optional.forEach(dep => {
      if (!this.dependencies[dep]) {
        console.warn(
          `[payment-serviceModule] Optional dependency '${dep}' not provided - related features will be disabled`,
        );
      }
    });
  }
  ...
  getConfig() {
    return {
      ...
      features: {
        ...
        notifications: !!this.dependencies.notification-service,
      },
      businessRules: {
        fees: {
  ...
  async healthCheck() {
    try {
      ...
      health.components.configuration = {
        ...
        notification-serviceAvailable: !!this.dependencies.notification-service,
      };
  ...
  async getStatisticsSummary(period = '30d') {
    try {
      ...
      logger.error('[payment-serviceModule] Statistics error:', error);
      throw error;
    }
  }
  ...
  async cleanupExpiredPayments() {
    try {
      logger.info('[payment-serviceModule] Starting expired payment cleanup');
      ...
      for (const payment of expiredPayments) {
        try {
          ...
          console.error(
            `[payment-serviceModule] Error cleaning up payment ${payment.paymentId}:`,
            error,
          );
        }
      }

      logger.info(`[payment-serviceModule] Cleanup completed: ${cleanupCount} payments expired`);
      ...
      logger.error('[payment-serviceModule] Cleanup error:', error);
      throw error;
    }
  }
  ...
  async notifyPaymentCompleted(paymentId) {
    try {
      ...
      logger.error('[payment-serviceModule] Payment notification error:', error);
      throw error;
    }
  }
   ...
   */
  async shutdown() {
    logger.info('[payment-serviceModule] Shutting down...');
    ...
    try {
      ...
      logger.info('[payment-serviceModule] Shutdown completed');
    } catch (error) {
      logger.error('[payment-serviceModule] Error during shutdown:', error);
      throw error;
    }
  }
}
...
module.exports = payment-serviceModule;
 ...
 * @returns {payment-serviceModule} Configured module instance
 */
module.exports.createModule = dependencies => {
  return new payment-serviceModule(dependencies);
};

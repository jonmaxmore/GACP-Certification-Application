...
const PaymentTransaction = require('../database/models/payment-transaction');
const KsherService = require('../services/services/ksher-service');
...
class ApplicationController {
    ...
    async createDraft(req, res) {
        try {
            console.log('[application-controller] createDraft called');
            console.log('[application-controller] req.user:', req.user);
            console.log('[application-controller] req.body:', JSON.stringify(req.body).substring(0, 500));
            ...
            if (!req.user || !req.user.id) {
                console.error('[application-controller] No user ID found in request');
            ...
            console.log('[application-controller] Draft created:', application._id);

            // Invalidate user's applications cache
            const RedisService = require('../services/services/redis-service');
            await RedisService.del(`apps:user:${req.user.id}`);

            res.status(201).json({ success: true, data: application });
        } catch (error) {
            console.error('[application-controller] createDraft error:', error.message, error.stack);
    ...
    async getMyApplications(req, res) {
        try {
            ...
            if (cached) {
                console.log('[application-controller] Cache HIT for', cacheKey);
                return res.json({
                    success: true,
                    data: cached.data,
                    total: cached.total,
            ...
            console.log('[application-controller] Cache SET for', cacheKey);
    ...
    async updateStatus(req, res) {
        try {
            ...
            console.log(`[application-controller] Status updated: ${prevStatus} -> ${status}`);
            res.json({ success: true, data: app, previousStatus: prevStatus });
        } catch (error) {
            console.error('[application-controller] updateStatus error:', error.message);
            res.status(500).json({ success: false, error: error.message });
        }
    }

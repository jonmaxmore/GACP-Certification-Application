  ...
  extractToken() {
    return (req, res, next) => {
      try {
        ...
        logger.error('[auth-middleware] Token extraction error:', error);
  ...
  authenticate() {
    return async (req, res, next) => {
      try {
        ...
        const userInfo = await this.auth-service.validateToken(req.token);
  ...
  authorize(permission) {
    return async (req, res, next) => {
      try {
        ...
        // Check permission
        const hasPermission = await this.auth-service.hasPermission(
          req.userId,
          permission,
          resourceContext,
        );
        ...
        logger.error('[auth-middleware] Authorization error:', error);
        return res.status(500).json({
          success: false,
  ...
  requireRole(roles) {
    ...
    return (req, res, next) => {
      try {
        ...
        logger.error('[auth-middleware] Role check error:', error);
        return res.status(500).json({
          success: false,
  ...
  validateResourceOwnership(resourceIdParam = 'id', ownerField = 'userId') {
    return async (req, res, next) => {
      try {
        ...
        logger.error('[auth-middleware] Resource ownership check error:', error);
        return res.status(500).json({
          success: false,
    ...
    return async (req, res, next) => {
      try {
        ...
        if (authHeader && authHeader.startsWith('Bearer ')) {
          ...
          if (token) {
            try {
              const userInfo = await this.auth-service.validateToken(token);
              req.user = userInfo;
              req.userId = userInfo.userId;
              req.userRole = userInfo.role;
              req.userPermissions = userInfo.permissions;
              ...
              logger.info('[auth-middleware] Optional auth failed:', error.message);
            }
          }
        }

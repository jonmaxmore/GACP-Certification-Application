...
class payment-service extends EventEmitter {
  constructor(paymentRepository, applicationRepository, notification-service) {
    ...
    this.notification-service = notification-service;
  ...
  async createPayment(data) {
    try {
      ...
      logger.info(`[payment-service] Creating ${type} payment for application ${applicationId}`);
      ...
      if (existingPayment && existingPayment.status === this.PAYMENT_STATUS.COMPLETED) {
        logger.warn(`[payment-service] Payment already completed for ${applicationId} (${type})`);
      ...
      logger.info(`[payment-service] Payment created: ${payment.id}`);
      return payment;
    } catch (error) {
      logger.error('[payment-service] Create payment error:', error);
      throw error;
    }
  }
  ...
  async completePayment(paymentId, data) {
    try {
      ...
      logger.info(`[payment-service] Completing payment ${paymentId}`);
      ...
      if (payment.status === this.PAYMENT_STATUS.COMPLETED) {
        logger.warn(`[payment-service] Payment already completed: ${paymentId}`);
        return payment;
      }
      ...
      logger.info(`[payment-service] Payment completed: ${paymentId}`);
      return updatedPayment;
    } catch (error) {
      logger.error('[payment-service] Complete payment error:', error);
      throw error;
    }
  }
  ...
  async isInitialPaymentRequired(applicationId) {
    try {
      ...
      logger.error('[payment-service] Check initial payment error:', error);
      throw error;
    }
  }
  ...
  async isResubmissionPaymentRequired(applicationId, submissionCount) {
    try {
      ...
      logger.error('[payment-service] Check resubmission payment error:', error);
      throw error;
    }
  }
  ...
  async getPaymentStatus(applicationId) {
    try {
      ...
      logger.error('[payment-service] Get payment status error:', error);
      throw error;
    }
  }
  ...
  async handleApplicationRejection(applicationId, farmerId, submissionCount) {
    try {
      logger.info(
        `[payment-service] Handling rejection for ${applicationId} (count: ${submissionCount})`,
      );
      ...
      logger.error('[payment-service] Handle rejection error:', error);
      throw error;
    }
  }
  ...
  async getFarmerPayments(farmerId) {
    try {
      ...
      logger.error('[payment-service] Get farmer payments error:', error);
      throw error;
    }
  }
  ...
  async cancelPayment(paymentId) {
    try {
      logger.info(`[payment-service] Cancelling payment ${paymentId}`);
      ...
      logger.error('[payment-service] Cancel payment error:', error);
      throw error;
    }
  }
  ...
  async getPaymentStatistics(filters = {}) {
    try {
      ...
      logger.error('[payment-service] Get statistics error:', error);
      throw error;
    }
  }
}

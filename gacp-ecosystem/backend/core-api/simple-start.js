...
try {
    ...
    const v2Routes = require('./src/routes/routes/v2');
    app.use('/api/v2', v2Routes);
    console.log('✅ V2 Routes loaded (includes auth-staff)');

    // Farmer Auth Routes
    const AuthFarmerRoutes = require('./src/routes/routes/api/auth-farmer-routes');
    app.use('/api/auth-farmer', AuthFarmerRoutes);
    app.use('/api/auth/farmer', AuthFarmerRoutes);
    console.log('✅ Auth Farmer Routes loaded');

    // Establishment Routes
    const EstablishmentRoutes = require('./src/modules/modules/establishment');
    app.use('/api/establishments', EstablishmentRoutes);
    app.use('/api/v2/establishments', EstablishmentRoutes);
    console.log('✅ Establishment Routes loaded');

    // Staff Management Routes
    const StaffRoutes = require('./src/routes/routes/api/staff-routes');
    app.use('/api/v2/staff', StaffRoutes);
    console.log('✅ Staff Routes loaded');

    // Accounting Routes
    const AccountingRoutes = require('./src/routes/routes/api/accounting-routes');
...
server = app.listen(port, '0.0.0.0', () => {
    ...
    const databaseService = require('./src/services/services/production-database');
    ...
    if (process.env.REDIS_ENABLED !== 'false') {
        const redisService = require('./src/services/services/redis-service');
